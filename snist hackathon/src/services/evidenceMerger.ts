import { DiscoveredSolution, Source, Feature } from "../graph/state";

export function mergeSolutionEvidence(
  existing: DiscoveredSolution | null,
  newEvidence: Partial<DiscoveredSolution>,
  sourceIds: string[]
): DiscoveredSolution {
  if (!existing) {
    return {
  ...
  for (const step of newEvidence.workflow || []) {
    const isDuplicate = allWorkflow.some(
      (w) =>
        w.action.toLowerCase() === step.action.toLowerCase() ||
  ...
  return {
    ...existing,
    description:
      newEvidence.description && newEvidence.description.length > existing.description.length
    ...
    targetUsers: mergeArrays(
      existing.targetUsers,
      newEvidence.targetUsers || []
    ),
    approach:
      newEvidence.approach && newEvidence.approach.length > existing.approach.length
    ...
    technologies: mergeArrays(
      existing.technologies,
      newEvidence.technologies || []
    ),
    limitations: mergeArrays(
      existing.limitations,
      newEvidence.limitations || []
    ),
    website:
      newEvidence.website || existing.website,
    githubRepository:
      newEvidence.githubRepository || existing.githubRepository,
...
export function mergeSearchResults(
  ...
  for (const r of tavilyResults) {
    ...
    if (seenUrls.has(normalized)) {
      ...
      if (r.content) source.content = r.content;
      seenUrls.set(normalized, source);
      sources.push(source);
    }
  }
