...
exports.validateProblemAnalysis = validateProblemAnalysis;
const state_1 = require("../../graph/state");
function validateProblemAnalysis(analysis) {
    const errors = [];
    const warnings = [];
    // First: Zod schema validation
    const zodResult = state_1.ProblemAnalysisSchema.safeParse(analysis);
    ...
    const questionCategories = new Set(data.researchQuestions.map((q) => q.category));
    if (questionCategories.size < 3) {
        warnings.push("Research questions should cover at least 3 different categories");
    }
    // Check research dimension coverage
    const dimNames = new Set(data.researchDimensions.map((d) => d.name));
    if (dimNames.size < 3) {
        warnings.push("Research dimensions should cover at least 3 different categories");
    }
    // Check analysis confidence
    if (data.analysisConfidence < 0.3) {
        warnings.push("Analysis confidence is quite low; consider adding more ambiguity notes");
    }
    return {
...
//# sourceMappingURL=strategist.validator.js.map