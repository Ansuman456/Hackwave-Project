...
exports.runInnovation = runInnovation;
const uuid_1 = require("uuid");
const sseStreamer_1 = require("../../utils/sseStreamer");
const vectorClustering_1 = require("./vectorClustering");
const gapAnalysis_1 = require("./gapAnalysis");
const candidateGenerator_1 = require("./candidateGenerator");
const candidateRanker_1 = require("./candidateRanker");
async function runInnovation(projectId, problemAnalysis, research) {
    (0, sseStreamer_1.emitEvent)(projectId, "innovation", "innovation_started", "Innovation agent starting analysis");
    try {
        ...
        (0, sseStreamer_1.emitEvent)(projectId, "innovation", "innovation_started", "Generating solution embeddings and clusters");
        let clusters;
        try {
            const embedded = await (0, vectorClustering_1.generateEmbeddings)(solutions);
            const rawClusters = (0, vectorClustering_1.clusterSolutions)(embedded, solutions);
            clusters = (0, vectorClustering_1.assignClusterNames)(rawClusters, solutions);
        }
        catch (err) {
        ...
        (0, sseStreamer_1.emitEvent)(projectId, "innovation", "innovation_started", "Analyzing feature landscape and identifying gaps");
        const featureLandscape = (0, gapAnalysis_1.buildFeatureLandscape)(solutions);
        const gaps = (0, gapAnalysis_1.identifyGaps)(featureLandscape, solutions, problemAnalysis.domainKeywords);
        // Step 3: Candidate Generation
        (0, sseStreamer_1.emitEvent)(projectId, "innovation", "innovation_started", "Generating candidate ideas");
        let candidates = await (0, candidateGenerator_1.generateCandidates)(problemAnalysis, research, gaps, featureLandscape, clusters);
        if (candidates.length === 0) {
            return { success: false, error: "Failed to generate any candidate ideas" };
        }
        // Step 4: Score & Filter Candidates
        (0, sseStreamer_1.emitEvent)(projectId, "innovation", "innovation_started", "Scoring and filtering candidates");
        const rawScored = (0, candidateRanker_1.scoreCandidates)(candidates, solutions);
        let scored = (0, candidateRanker_1.filterDuplicateCandidates)(rawScored, solutions);
        ...
        const closestSolutions = (0, candidateRanker_1.getClosestExistingSolutions)(scored[0], solutions);
        ...
        const innovationResult = {
            innovationId: `inn_${(0, uuid_1.v4)().substring(0, 8)}`,
        ...
        (0, sseStreamer_1.emitEvent)(projectId, "innovation", "innovation_completed", `Innovation analysis complete: ${scored.length} candidates generated, top score: ${scored[0].overallConceptScore}`);
        ...
        (0, sseStreamer_1.emitEvent)(projectId, "innovation", "agent_failed", msg);
        return { success: false, error: msg };
    }
}
function buildFallbackClusters(solutions) {
...
function extractDominantApproaches(solutions) {
...
function buildDifferentiationSummary(candidate, closestSolutions, allSolutions) {
    const closest = closestSolutions[0];
    if (!closest)
        return "No close existing solutions found for comparison.";
...
function assessNovelty(candidate, solutions, featureLandscape) {
    const candidateFeatureNames = new Set(candidate.keyFeatures.map((f) => f.name.toLowerCase().trim()));
    ...
    let classification;
    if (overlapRatio < 0.2)
        classification = "high_differentiation";
    else if (overlapRatio < 0.5)
        classification = "moderate_differentiation";
    else
        classification = "low_differentiation";
    return {
        classification,
        ...
        strongestDifferentiators: candidate.differentiators?.map((d) => d.statement) || [],
        majorOverlapAreas: [],
...
function deriveCapabilityRequirements(candidate) {
    const capabilities = new Set();
    const techCapabilities = new Set();
    const roles = new Set();
    for (const feature of candidate.keyFeatures || []) {
        capabilities.add(feature.name);
        if (feature.category === "ai")
            techCapabilities.add("AI/ML model");
        if (feature.category === "automation")
            techCapabilities.add("automation engine");
        if (feature.category === "analytics")
            techCapabilities.add("analytics dashboard");
        if (feature.category === "workflow")
            techCapabilities.add("workflow engine");
    }
...
function generateValidationQuestions(candidate, gaps) {
    const questions = [];
...
function calculateConfidence(candidates, solutions) {
    if (candidates.length === 0)
        return 0;
...
//# sourceMappingURL=innovation.agent.js.map