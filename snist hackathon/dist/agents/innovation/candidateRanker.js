...
exports.filterDuplicateCandidates = filterDuplicateCandidates;
exports.getClosestExistingSolutions = getClosestExistingSolutions;
const WEIGHTS = {
    problemFit: 0.15,
...
function scoreCandidates(candidates, existingSolutions) {
    const scored = candidates.map((c) => {
        // Calculate composite score from individual scores
        const overallConceptScore = c.innovationScore * WEIGHTS.problemFit +
...
function filterDuplicateCandidates(candidates, existingSolutions, similarityThreshold = 0.7) {
    const filtered = [];
    for (const candidate of candidates) {
        // Check against existing solutions
        const isTooSimilar = existingSolutions.some((sol) => {
            const overlap = calculateFeatureOverlap(candidate.keyFeatures.map((f) => f.name), sol.features.map((f) => f.name));
    ...
    for (const candidate of filtered) {
        const isDuplicate = uniqueCandidates.some((existing) => {
            const overlap = calculateFeatureOverlap(candidate.keyFeatures.map((f) => f.name), existing.keyFeatures.map((f) => f.name));
            return overlap > similarityThreshold;
        });
        if (!isDuplicate) {
...
function calculateFeatureOverlap(featuresA, featuresB) {
...
function getClosestExistingSolutions(candidate, existingSolutions, topN = 3) {
    const candidateFeatures = new Set(candidate.keyFeatures.map((f) => f.name.toLowerCase().trim()));
