import { DiscoveredSolution, FeatureLandscape, InnovationGap, Feature } from "../../graph/state";
...
export function identifyGaps(
  ...
  const hasAutomation = solutions.some((s) =>
    s.features.some((f) => f.category === "automation")
  );
  const hasWorkflow = solutions.some((s) =>
    s.features.some((f) => f.category === "workflow")
  );
  if (!hasAutomation || !hasWorkflow) {
    gaps.push({
      ...
      description:
        "Most solutions require manual intervention. An automated end-to-end workflow is a significant opportunity.",
  ...
  for (const constraint of constraintKeywords) {
    const affected = solutions.filter((s) =>
      s.limitations.some((l) => l.toLowerCase().includes(constraint))
    );
...
export function calculateFeatureFrequency(
  landscape: FeatureLandscape[]
): { common: string[]; moderate: string[]; rare: string[] } {
  return {
